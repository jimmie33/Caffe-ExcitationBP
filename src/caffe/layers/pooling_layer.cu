...
template <typename Dtype>
void PoolingLayer<Dtype>::Backward_eb_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  if (!propagate_down[0]) {
    return;
  }
  ...
  Blob<Dtype> tsum(top[0]->shape());
  Dtype* tsum_data = tsum.mutable_gpu_data();
  switch (this->layer_param_.pooling_param().pool()) {
  case PoolingParameter_PoolMethod_MAX:
    if (use_top_mask) {
      top_mask = top[1]->gpu_data();
  ...
  case PoolingParameter_PoolMethod_AVE:
    // NOLINT_NEXT_LINE(whitespace/operators)
    SumPoolForward<Dtype><<<CAFFE_GET_BLOCKS(tsum.count()), CAFFE_CUDA_NUM_THREADS>>>(
        tsum.count(), bottom_data, bottom[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_, kernel_h_,
        kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, tsum_data);
        
    AvePoolBackwardEB<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff, tsum_data, bottom_data, top[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_, kernel_h_,
        kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, bottom_diff);
    break;
  case PoolingParameter_PoolMethod_STOCHASTIC:
...
INSTANTIATE_LAYER_EB_GPU_FUNCS(PoolingLayer);
